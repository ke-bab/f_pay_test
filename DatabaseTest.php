...
//            'SELECT name FROM users WHERE `user_id` IN (1, 2, 3) AND block = 1',
        ...
        if ($results !== $correct) {
            throw new Exception("results are not equal expected value.\n" . self::printArrays($correct, $results));
        }
    }

    private static function printArrays(array $want, array $got): string
    {
        $str = "want:\n";
